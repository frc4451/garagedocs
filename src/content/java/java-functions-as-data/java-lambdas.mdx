...
Both versions apply the same test here. A lambda supplies an implementation of a functional interface; it is not a textual rewrite into an anonymous class.

<RulesBox title="The interface supplies the type">

A lambda needs a target functional interface. Here, `ReadingTest` tells Java the parameter type and return type.

Lambdas and anonymous classes differ in scope and object identity. For example, `this` inside a lambda means the enclosing object, as the example below shows. Do not rely on two lambda evaluations creating two different objects.
...
One difference from an anonymous class is the meaning of `this`.
...
- **The body is long.** A lambda spanning fifteen lines is harder to read than a named method. Extract it and use a [method reference](/java/fundamentals/java-method-references#when-a-lambda-forwards-directly).
- **You need a constructor or fields.** Use a real class.
- **You need recursion.** A lambda has no straightforward way to refer to itself.
