---
title: Supplier and the Functional Interfaces
lessonId: java-fn-supplier
section: java
...
For ordinary code the boxed versions are fine and clearer. Reach for the primitive ones only when profiling shows the boxing matters, or when a library asks for them: WPILib's `Trigger` takes a `BooleanSupplier`, and its joystick and motor APIs hand out `DoubleSupplier`s, because a robot loop is exactly the hot loop the primitive versions exist for.
...
<JavaPlayground id="java-fn-supplier" />
