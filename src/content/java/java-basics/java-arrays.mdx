...
An array is a fixed number of numbered slots for values. The number of slots is its **length**, set when the array is created, and each slot's number is its **index**, counting from 0. The values in the slots are called **elements**. An array can hold primitives (`int`, `double`, `boolean`) or objects (`String`, your own classes).
