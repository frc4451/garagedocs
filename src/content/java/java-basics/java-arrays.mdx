...
An array is a fixed number of numbered slots for values. The number of slots is its **length**, which is set when the array is created.

Each slot's number is its **index**, starting at 0. The values stored in those slots are **elements**. An array can hold primitives (`int`, `double`, `boolean`) or objects (`String`, your own classes).
...
An array can hold objects as well as primitives. String arrays are the common case, but the element type can be any class, including your own.
...
- **Null checks:** an object array starts full of `null`, so check an element before calling a method on it
- **Bounds:** an index must be between 0 and `length - 1`, or the access throws
- **For-each loops:** use one when you do not need the index
- **`.length`:** a field on arrays, not a method; `.length()` is for strings
- **Names:** name the array for what it holds, such as `motors` or `teamNames`
