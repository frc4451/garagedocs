...
Among these arithmetic operators, `*`, `/`, and `%` have higher precedence than `+` and `-`. Operators at the same level are evaluated left to right. Use parentheses to control the order of operations.
...
- Cast to `(double)` before division to keep a fractional result; floating-point arithmetic can still round
- Expect integer division to drop the fractional part
...
- Dividing an integer by zero; it throws `ArithmeticException`
...
1. With length 4.5 m and width 2.0 m, print area 9.0 square meters.
2. Print 7 / 2 and 7.0 / 2. Explain why one loses the fraction.
...
<Sources items={[
  ...
  {"source": "java-tutorial", "path": "java/nutsandbolts/opsummary.html", "note": "Operator summary."},
  {"source": "dev-java", "path": "language-basics/expressions-statements-blocks/", "note": "Expressions and statements."},
  {"source": "jls", "path": "jls-15.html", "note": "Operators, precedence, integer division (chapter 15)."},
  {"source": "jls", "path": "jls-5.html", "note": "Conversions and casts (chapter 5)."},
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Console Input", "id": "java-console-input"}]} />
