...
Both take a `double`, so they have the same signature. Java cannot distinguish them, and no amount of [overloading](/java/fundamentals/overloading#constructor-overloading) helps.
...
This is the form that pairs with [dependency injection](/java/fundamentals/dependency-injection#injecting-an-interface): you pass a factory to code that needs to create things without deciding how they are configured.
...
[Runtime polymorphism](/java/fundamentals/runtime-polymorphism#a-worked-example) does the dispatch. The decision about which class to build is made in exactly one place, and every user of `Drivetrain` is unaffected by it.
