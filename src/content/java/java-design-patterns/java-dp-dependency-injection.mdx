...
`Shooter` now works with anything satisfying that interface: a vendor's motor class, a simulated one, or a recording one used for diagnostics. It does not know or care, thanks to [runtime polymorphism](/java/java-runtime-polymorphism#two-kinds-of-type).
...
The same reasoning applies to plain values and to [suppliers](/java/java-fn-supplier#lazy-and-expensive-work).
...
<Sources items={[
  ...
  {"url": "/frc/io-layer", "label": "The IO layer on the robot", "note": "Injection of hardware interfaces in FRC 4451's code."},
