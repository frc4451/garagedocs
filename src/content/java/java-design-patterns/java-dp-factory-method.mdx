...
Both take a `double`, so they have the same signature. Java cannot distinguish them, and no amount of [overloading](/java/java-overloading#constructor-overloading) helps.
...
This is the form that pairs with [dependency injection](/java/java-dp-dependency-injection#injecting-an-interface): you pass a factory to code that needs to create things without deciding how they are configured.
...
[Runtime polymorphism](/java/java-runtime-polymorphism#a-worked-example) does the dispatch. The decision about which class to build is made in exactly one place, and every user of `Drivetrain` is unaffected by it.
...
A factory for a class with one simple constructor is overhead with no benefit. `new Point(3, 4)` is perfectly clear, and wrapping it in `Point.of(3, 4)` adds a layer for nothing.
