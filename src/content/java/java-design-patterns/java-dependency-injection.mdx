...
`Shooter` now works with anything satisfying that interface: a vendor's motor class, a simulated one, or a recording one used for diagnostics. It does not know or care, thanks to [runtime polymorphism](/java/fundamentals/runtime-polymorphism#two-kinds-of-type).
...
The same reasoning applies to plain values and to [suppliers](/java/fundamentals/functional-interfaces#lazy-and-expensive-work).
