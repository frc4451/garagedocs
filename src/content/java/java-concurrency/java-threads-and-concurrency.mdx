...
Methods such as join clear the interrupt status when they throw InterruptedException. Restore the flag when you catch the exception without handling the interruption fully, so calling code can still detect it.
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Race Conditions and Synchronization", "id": "java-race-conditions-and-synchronization"}]} />
