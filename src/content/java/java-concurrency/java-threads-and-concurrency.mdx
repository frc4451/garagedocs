...
- **Shared state.** Two threads touching the same variable can corrupt it, even for operations that look atomic. See [Race Conditions](/java/fundamentals/race-conditions-and-synchronization#a-counter-that-loses-count).
...
- **Creating threads in a loop.** Each carries real cost; use a pool. See [Executors and Thread Pools](/java/fundamentals/executors-and-thread-pools#the-problem-with-new-thread).
