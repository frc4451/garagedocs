...
lessonId: java-executors-and-thread-pools
section: java
...
If a repeating task throws, later executions are suppressed. Keep its ScheduledFuture so a caller can inspect completion and retrieve the failure. Catch and report an exception inside the task only when continuing is appropriate for that failure.
...
- **`get()` immediately after `submit()`**, which makes the work sequential and adds executor overhead.
...
<JavaPlayground id="java-executors-and-thread-pools" />
...
<LinkGrid
  ...
  links={[
    ...
    { "label": "Supplier and the Functional Interfaces", "id": "java-functional-interfaces" }
  ]}
/>

<LinkGrid title="Next lesson" section="java" links={[{"label": "Concurrent Collections", "id": "java-concurrent-collections"}]} />
