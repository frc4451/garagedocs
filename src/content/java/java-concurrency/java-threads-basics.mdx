...
- **Shared state.** Two threads touching the same variable can corrupt it, even for operations that look atomic. See [Race Conditions](/java/java-race-conditions#a-counter-that-loses-count).
...
The FTC SDK and WPILib both call your code on a fixed loop, usually every 20 milliseconds. That loop is already the concurrency model, and adding threads is usually the wrong instinct.
...
- **Creating threads in a loop.** Each carries real cost; use a pool. See [Executors and Thread Pools](/java/java-executors#the-problem-with-new-thread).

</RulesBox>

<JavaPlayground id="java-threads-basics" />
