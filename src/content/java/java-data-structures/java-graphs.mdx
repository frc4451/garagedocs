...
A [tree](/java/fundamentals/trees#branching-instead-of-chaining) has a root, and every node has exactly one parent. A graph drops both rules. Any node may connect to any other, connections can form loops, and there is no top.
...
Explore all immediate neighbours first, then their neighbours, spreading outward in rings. Uses a [queue](/java/fundamentals/queues#first-in-first-out).
