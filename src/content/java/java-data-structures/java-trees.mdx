...
A [linked list](/java/fundamentals/linked-lists#a-different-arrangement) node points at one other node. A tree node points at several. That single change gives you a structure for anything hierarchical.
...
Level-order is the odd one out; it needs a [queue](/java/fundamentals/queues#using-one) rather than recursion:
