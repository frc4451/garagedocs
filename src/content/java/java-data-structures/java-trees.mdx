...
<ReferenceImage
  src="/media/java/binary-tree.svg"
  alt="Binary tree with a root node and children arranged in levels"
  caption="A tree starts at one root and branches into child nodes. Each node in this binary tree has at most two children."
  sourceUrl="https://commons.wikimedia.org/wiki/File:Binary_tree.svg"
  sourceLabel="Wikimedia Commons, public domain"
  contain
/>

A [linked list](/java/fundamentals/java-linked-lists#a-different-arrangement) node points at one other node. A tree node points at several. That single change gives you a structure for anything hierarchical.
...
Level-order is the odd one out; it needs a [queue](/java/fundamentals/java-queues#using-one) rather than recursion:
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Graphs", "id": "java-graphs"}]} />
