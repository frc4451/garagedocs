...
On a robot, an unhandled exception in robot code stops the program, and the robot is disabled until it restarts. Catching an exception where a fallback exists (a default value, a skipped step, a logged message) keeps the rest of the program running. Catching one where no fallback exists only hides the problem; the section on multiple catch blocks below shows how to be specific.
