...
Trace fib(5): fib(3) is computed twice, fib(2) three times, and fib(1) five times. Larger inputs repeat more work; fib(40) makes hundreds of millions of calls.
...
With a cache, each distinct Fibonacci value is calculated once. The work grows linearly with n, with additional calls returning cached values.
...
<RulesBox title="When caching helps">

**Repeated inputs:** several calls request the same subproblem. Reusing its result can avoid repeated work.

**Reusable results:** the stored result remains valid for those inputs. If a result also depends on changing state, include that state in the key or clear the cache when it changes.

Dynamic programming combines related subproblem results. Optimization problems often require optimal substructure; counting problems such as Fibonacci use a recurrence without choosing an optimal solution.
...
```java
public static long fibTwoVars(int n) {
    if (n <= 1) return n;
...
```java
public static long countPaths(char[][] grid) {
...
Step 5 catches most errors. If `dp[3]` disagrees with what you counted on paper, check both the recurrence and its implementation.
...
- **`int` overflow.** Counting problems grow fast; use `long` where it fits, and check its range too.
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Factory Methods", "id": "java-factory-methods"}]} />
