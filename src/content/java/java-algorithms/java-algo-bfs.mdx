...
duration: 25 min
description: Exploring outward in rings from a starting point, why that finds shortest paths, and how to recover the route rather than just the distance.
---

### Spreading outward

Breadth-first search visits everything one step away, then everything two steps away, and so on. It uses a [queue](/java/java-ds-queues#first-in-first-out) rather than recursion, which is what makes it spread sideways instead of diving deep.
...
This is the property [depth-first search](/java/java-algo-dfs#dfs-or-bfs) lacks. DFS reaches nodes in whatever order its recursion happens to take, so its first arrival is not necessarily the shortest.
...
Everything else is unchanged, and one pass gives every cell its distance to the closest source. The trick is not obvious the first time and comes up often.
...
<ContentTable
  ...
  rows={[
    ...
    ["Just reach everything", "Either"],
    ["Cycle detection", "DFS"],
    ["Explore all arrangements", "DFS with undo"],
    ["Distance from nearest of several sources", "Multi-source BFS"],
    ["Varying edge costs", "Neither; <a href=\"/java/java-algo-shortest-paths\">Dijkstra</a>"]
  ]}
/>
...
<JavaPlayground id="java-algo-bfs" />
...
<LinkGrid
  ...
  links={[
    ...
    {"label":"Memoization and Dynamic Programming","id":"java-algo-memoization"}
  ]}
/>
