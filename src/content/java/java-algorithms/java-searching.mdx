...
difficulty: beginner
duration: 30 min
description: Finding a value by checking every element versus repeatedly halving a sorted range, including the boundary details required for correct binary search.
---
...
A more useful binary-search pattern finds where a condition changes. Suppose a list is sorted and you want the first element at or above a threshold:
...
public class Main {
    ...
    public static boolean holds(double volts) {
        return volts >= 1.85;
    }

    // Smallest voltage that holds, to within tolerance.
    public static double smallestHolding(double low, double high, double tolerance) {
...
Sorting costs more than a single linear scan. Sorting so you can binary search *once* is slower than a direct scan.
...
<JavaPlayground id="java-searching" />
...
<LinkGrid
  ...
  section="java"
  links={[
    {"label":"Sorting Algorithms","id":"java-sorting-algorithms"},
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Sorting Algorithms", "id": "java-sorting-algorithms"}]} />
