...
Assume the node type from [Trees](/java/fundamentals/java-trees#branching-instead-of-chaining):
...
The odd one out. It cannot be done with plain recursion because it moves across the tree rather than down it. Use a [queue](/java/fundamentals/java-queues#using-one):
...
This is [breadth-first search](/java/fundamentals/java-breadth-first-search#spreading-outward) applied to a tree.
...
Recursion is clearer, but a very deep tree can overflow the call stack. An explicit [stack](/java/fundamentals/java-stacks#which-class-to-use) removes that limit.
...
<JavaPlayground id="java-tree-traversals" />
...
<LinkGrid title="Next lesson" section="java" links={[{"label": "Depth-First Search", "id": "java-depth-first-search"}]} />
