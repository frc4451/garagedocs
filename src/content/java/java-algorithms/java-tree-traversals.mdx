...
Assume the node type from [Trees](/java/fundamentals/trees#branching-instead-of-chaining):
...
The odd one out. It cannot be done with plain recursion because it moves across the tree rather than down it. Use a [queue](/java/fundamentals/queues#using-one):
...
This is [breadth-first search](/java/fundamentals/breadth-first-search#spreading-outward) applied to a tree.
...
Recursion is clearer, but a very deep tree can overflow the call stack. An explicit [stack](/java/fundamentals/stacks#which-class-to-use) removes that limit.
