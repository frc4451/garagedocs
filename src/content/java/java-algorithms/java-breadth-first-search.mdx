...
Breadth-first search visits everything one step away, then everything two steps away, and so on. It uses a [queue](/java/fundamentals/java-queues#first-in-first-out) rather than recursion, which is what makes it spread sideways instead of diving deep.
...
This is the property [depth-first search](/java/fundamentals/java-depth-first-search#dfs-or-bfs) lacks. DFS reaches nodes in whatever order its recursion happens to take, so its first arrival is not necessarily the shortest.
...
<ContentTable
  ...
  rows={[
    ...
    ["Varying edge costs", "Neither; <a href=\"/java/fundamentals/java-shortest-paths\">Dijkstra</a>"]
  ]}
/>
