...
This is a literal [stack](/java/java-ds-stacks#last-in-first-out), the same last-in-first-out structure, which is why running out of space is a `StackOverflowError`.
...
- **The data structure is recursive.** A [tree](/java/java-ds-trees#recursive-measurements) node contains subtrees, which are trees. Recursion mirrors the shape.
...
Two recursive calls, and the structure handles the bookkeeping. Most tree and graph work looks like this; see [Tree Traversals](/java/java-algo-tree-traversals#four-orders) and [Depth-First Search](/java/java-algo-dfs#going-deep-first).
...
This is correct but recomputes the same values constantly; `fib(40)` makes over a billion calls. The fix is to remember results you have already computed, covered in [Memoization and Dynamic Programming](/java/java-algo-memoization#memoization).
...
- **Modifying shared state without undoing it.** See [Backtracking](/java/java-algo-backtracking#choose-recurse-undo).
- **Assuming recursion is free.** Each call costs a stack frame.
- **Recomputing the same sub-problem** exponentially.

</RulesBox>

<JavaPlayground id="java-algo-recursion" />
