---
title: "Interfaces"
lessonId: java-interfaces
section: java
group: java-oop
groupLabel: "Object-Oriented Programming"
groupOrder: 4
order: 8
difficulty: intermediate
duration: 30 min
description: Declaring a capability that any class can claim, implementing several at once, and the default and static methods interfaces gained in modern Java.
---

### Describing a capability

An interface lists methods a class promises to provide, without saying how.

```java
public interface Stoppable {
    public void stop();
}
```

Any class can claim that capability, regardless of what else it is:

```java norun
public class Intake extends Subsystem implements Stoppable {
    @Override
    public void stop() {
        motor.set(0);
    }
}

public class Camera implements Stoppable {          // unrelated to Subsystem
    @Override
    public void stop() {
        stopRecording();
    }
}
```

`Intake` and `Camera` share no ancestor, but both are `Stoppable`, so code can handle them together:

```java norun
List<Stoppable> everything = List.of(intake, camera);
for (Stoppable s : everything) {
    s.stop();
}
```

That is the thing [inheritance](/java/fundamentals/java-inheritance#java-has-single-inheritance) cannot do. A class extends one parent, but implements as many interfaces as it needs.

### Implementing several

```java norun
public interface Stoppable  { public void stop(); }
public interface Loggable   { public String toLogLine(); }

public class Intake extends Subsystem implements Stoppable, Loggable {
    @Override public void stop() { motor.set(0); }
    @Override public String toLogLine() { return "intake: " + motor.get(); }
}
```

`extends` comes first and names one class; `implements` follows and names any number of interfaces.

<RulesBox title="Interface methods are public">

The abstract interface methods shown here are public, and their implementations must be public too. Interfaces can also contain private helper methods with bodies. A private implementation of a public requirement is invalid:

```java norun
@Override
private void stop() { }             // weaker access: will not compile
```

fails with "attempting to assign weaker access privileges". Java will not let an implementation be less visible than the interface promises. Always write `public` on interface implementations.

</RulesBox>

### Default methods

Originally every interface method was abstract. Modern Java lets an interface supply a body with `default`:

```java
public interface Stoppable {
    public void stop();

    public default void emergencyStop() {
        System.out.println("emergency stop requested");
        stop();                              // may call abstract methods
    }
}
```

Implementations get `emergencyStop` for free and may override it if they want something different.

A default method supplies behavior that an implementation can inherit. It can let an interface gain an operation without requiring each class to implement it. Conflicting inherited defaults can still require an explicit override.

<RulesBox title="Do not treat default methods as a replacement for abstract classes">

An interface has no instance fields of its own. A default method can use parameters, call other methods, and access interface constants.

When shared behaviour needs shared *data*, you still want an [abstract class](/java/fundamentals/java-abstract-classes#abstract-class-or-interface).

</RulesBox>

### Static and private methods

An interface may also hold static helpers:

```java norun
public interface Stoppable {
    public void stop();

    public static void stopAll(List<Stoppable> items) {
        for (Stoppable s : items) s.stop();
    }
}

Stoppable.stopAll(everything);
```

Static interface methods are **not** inherited by implementations; call them on the interface name.

Private interface methods also exist, purely so several `default` methods can share code without exposing it.

### Constants

Fields in an interface are implicitly `public static final`:

```java
public interface RobotLimits {
    public static final double MAX_SPEED = 4.5;              // public static final
}
```

This works but is generally poor practice. An interface should describe behaviour; a bag of constants is better as a `final` class with a private constructor, or an `enum`.

### Functional interfaces

An interface with exactly one abstract method can be implemented by a [lambda](/java/fundamentals/java-lambdas#the-same-thing-shorter):

```java
@FunctionalInterface
public interface ReadingTest {
    public boolean matches(int reading);
}

ReadingTest above100 = r -> r > 100;      // no class needed
```

`default` and `static` methods do not count toward the "exactly one" rule; only abstract ones do. The `@FunctionalInterface` annotation is optional but makes the compiler reject a second abstract method, which would silently break every lambda written against it.

This is the mechanism behind the whole [Functions as Data](/java/fundamentals/java-functions-as-data#functional-interfaces) module.

### Interface or abstract class

<ContentTable
  headers={["Need", "Use"]}
  rows={[
    ["A capability unrelated classes might share", "Interface"],
    ["A class to fill several roles", "Interface"],
    ["Shared fields or constructor logic", "Abstract class"],
    ["A fixed sequence with subclass-supplied steps", "Abstract class"],
    ["Something a lambda can implement", "Interface with one abstract method"],
    ["Both would work", "Interface; it costs nothing to add later"]
  ]}
/>

The last row is the practical tie-breaker. A class extends only one class, so spending that slot is a real commitment; implementing an interface is not.

### Naming

Two conventions worth following. Capabilities often end in *-able*: `Stoppable`, `Comparable`, `Runnable`. Roles are plain nouns: `List`, `Command`, `MotorController`.

Do not prefix with `I` (`IStoppable`). That convention comes from other ecosystems and looks out of place in Java.

### Programming to the interface

The main practical habit interfaces enable:

```java
List<String> names = new ArrayList<>();          // declared as the interface
Map<String, Integer> ports = new HashMap<>();
```

Declaring the variable as the interface means switching implementation later touches one line. It also means methods taking `List` accept any list, rather than only `ArrayList`.

The same reasoning drives [dependency injection](/java/fundamentals/java-dependency-injection#injecting-an-interface): accept an interface, and callers decide what to supply, including a fake for testing.

<RulesBox title="Common mistakes">

- **Forgetting `public`** on an implementation.
- **Expecting instance fields.** Interfaces have none.
- **Expecting a constructor.** Interfaces have none.
- **Calling a static interface method on an instance.** Use the interface name.
- **Adding a second abstract method** to a functional interface, breaking lambdas.
- **Using an interface as a constants holder.**
- **`IStoppable`-style naming.**
- **Declaring variables as `ArrayList`** rather than `List`, losing the flexibility.

</RulesBox>

<JavaPlayground id="java-interfaces" />

<ExerciseBox title="Practice">

1. Write a `Stoppable` interface and implement it in two classes that share no parent. Loop a mixed list and stop each.
2. Write a class implementing two interfaces at once and confirm it satisfies both.
3. Add a `default` method to an interface, then override it in one implementation and not the other. Confirm which runs.
4. Write a functional interface and implement it with a lambda. Then add a second abstract method and read the compile error.
5. Take a class with a hard-coded dependency and rewrite it to accept an interface, then test it with a fake implementation.

<details>
<summary>Hints</summary>

1. `List<Stoppable>` holding both.
2. `implements A, B` and implement every method from each.
3. The one that does not override inherits the default.
4. With `@FunctionalInterface` the error is clear; without it, the failure shows up at the lambda instead.
5. This is dependency injection. The fake records what it was told so the test can check it.

</details>

</ExerciseBox>

<LinkGrid
  title="Related"
  section="java"
  links={[
    { "label": "Runtime Polymorphism", "id": "java-runtime-polymorphism" },
    { "label": "Passing Functions as Data", "id": "java-functions-as-data" }
  ]}
/>

<Sources items={[
  {"source": "csce146", "note": "The course whose structure this lesson follows."},
  {"source": "dev-java", "path": "interfaces/", "note": "Interfaces."},
  {"source": "dev-java", "path": "interfaces/defining-interfaces/", "note": "default and static methods."},
  {"source": "jls", "path": "jls-9.html", "note": "Interfaces (chapter 9)."},
  {"source": "java-api", "path": "java.base/java/lang/FunctionalInterface.html", "note": "<code>@FunctionalInterface</code>."},
  {"source": "java-api", "path": "java.base/java/lang/Comparable.html", "note": "<code>Comparable</code> API reference."},
  {"source": "java-api", "path": "java.base/java/lang/Runnable.html", "note": "<code>Runnable</code> API reference."},
  {"source": "java-api", "path": "java.base/java/util/ArrayList.html", "note": "<code>ArrayList</code> API reference."},
  {"source": "java-api", "path": "java.base/java/util/List.html", "note": "<code>List</code> API reference."}
]} />

<LinkGrid
  title="Related"
  section="java"
  links={[
    { "label": "Inheritance", "id": "java-inheritance" },
    { "label": "Abstract Classes and Methods", "id": "java-abstract-classes" },
    { "label": "Dependency Injection", "id": "java-dependency-injection" }
  ]}
/>

<LinkGrid title="Next lesson" section="java" links={[{"label": "Runtime Polymorphism", "id": "java-runtime-polymorphism"}]} />
