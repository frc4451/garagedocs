...
Which version actually runs is decided at runtime by the real object, not the declared type. That is [runtime polymorphism](/java/java-runtime-polymorphism#two-kinds-of-type), and it is the reason inheritance is useful rather than merely tidy.
...
This avoids the ambiguity that arises when two parents define the same method differently. When you need a class to fill several roles, use [interfaces](/java/java-interfaces#implementing-several); a class can implement any number of those.
...
Composition (holding an object as a field) is looser, easier to test, and easier to change. Reach for inheritance when there is a genuine is-a relationship *and* real behaviour to share. Reach for composition otherwise. See [Dependency Injection](/java/java-dp-dependency-injection#passing-it-in-instead).
