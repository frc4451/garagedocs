...
Web-based programming and robotics learning platform for FRC 4451: Tools, Java, Practice, Kit Bot and FRC.
...
listed under "Who wrote what" on the homepage and in `NOTICE`. Its Java fences are WPILib code and are deliberately
...
The homepage (`src/content/homepage/index.mdx`) ends with a **References** section listing
the curriculum sources, the documentation lessons are written against, and what the site is
built with. Add to it whenever a lesson starts leaning on a new source.
...
- **Resources** at `/resources` is a read-only catalog. Add approved links by
  ...
  a lesson relies on should have an entry, and the homepage References section should list it.
